...
public class Pomodoro {
    ...
    public static void main(String[] args) throws InterruptedException {
        System.out.println("Программа распределения рабочего времени - Pomodoro. Напиши пожалуйста команду в формате -w <Время работы> -b <Время отдыха> -m <кол-во подходов>");
        ...
        if (!isCallHelp) {
            System.out.printf("Работа %d min, " +
                    "отдых %d min, кол-во подходов %d\n", workMin, breakMin, m);
            long startTime = System.currentTimeMillis();
            for (int i = 1; i <= m; i++) {
                System.out.printf("Выполняем итерацию %d\n", i);
    ...
    private static void timer(int workTime, int breakTime, int sizeProgressBar) throws InterruptedException {
        ...
        printProgress("Время отдыха:: ", breakTime, sizeProgressBar);
    }
